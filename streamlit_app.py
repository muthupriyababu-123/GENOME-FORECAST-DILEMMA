...
<style>
    ...
    .stApp { background: linear-gradient(160deg, #f0faf0 0%, #e8f5e9 50%, #f9fbe7 100%); }
    ...
    .hero p  { font-size: 1rem; opacity: 0.85; margin: 0; color: white !important; }
    .section-card {
        background: white; border-radius: 16px; padding: 28px;
        margin-bottom: 20px; box-shadow: 0 2px 16px rgba(0,0,0,0.07);
        border: 1px solid #e8f5e9;
    }
    .section-title { font-size: 1.1rem; font-weight: 700; color: #1b5e20; margin-bottom: 6px; }
    .section-subtitle { font-size: 0.82rem; color: #888; margin-bottom: 18px; }
    ...
    .metric-label { font-size: 0.75rem; color: #666; margin-top: 2px; font-weight: 600; text-transform: uppercase; letter-spacing: 0.04em; }
    .info-box    { background: #e3f2fd; border-left: 4px solid #1565c0; border-radius: 8px; padding: 12px 16px; font-size: 0.83rem; color: #0d47a1; margin: 12px 0; }
    .warn-box    { background: #fff8e1; border-left: 4px solid #f9a825; border-radius: 8px; padding: 12px 16px; font-size: 0.83rem; color: #e65100; margin: 12px 0; }
    .success-box { background: #e8f5e9; border-left: 4px solid #2e7d32; border-radius: 8px; padding: 12px 16px; font-size: 0.83rem; color: #1b5e20; margin: 12px 0; }
    .stButton > button {
    ...
    .stButton > button:hover { transform: translateY(-1px) !important; box-shadow: 0 6px 20px rgba(46,125,50,0.4) !important; }
...
# =====================================================================
# SESSION STATE INIT — keeps results alive when selectbox changes
# =====================================================================
if 'df_result' not in st.session_state:  st.session_state.df_result = None
if 'X_result'  not in st.session_state:  st.session_state.X_result  = None
if 'sv_result' not in st.session_state:  st.session_state.sv_result = None
...
def make_shap_plot(sv):
    ...
    bars = ax.barh(feature_importance.index, feature_importance.values,
                   color='#43a047', height=0.55, edgecolor='white', linewidth=1.5)
    for bar, val in zip(bars, feature_importance.values):
        ax.text(val + 0.002, bar.get_y() + bar.get_height()/2,
                f'{val:.3f}', va='center', ha='left', fontsize=9, fontweight='600', color='#333')
    ax.set_xlabel('Mean |SHAP Value| — Average Impact on Yield (t/ha)', fontsize=9, color='#555')
    ax.set_title('Feature Importance (SHAP)', fontsize=12, fontweight='800', color='#1b5e20', pad=12)
...
def make_individual_shap_plot(shap_values, feature_names, sample_id):
    ...
    bars = ax.barh(labels, values, color=colors, height=0.5, edgecolor='white', linewidth=1)
    for bar, val in zip(bars, values):
        x_pos = val + (0.003 if val >= 0 else -0.003)
        ha    = 'left' if val >= 0 else 'right'
        ax.text(x_pos, bar.get_y() + bar.get_height()/2,
                f'{val:+.3f}', va='center', ha=ha, fontsize=8.5, fontweight='600',
    ...
    ax.set_title(f'Feature Contributions — {sample_id}', fontsize=10, fontweight='700', color='#1b5e20')
    ax.spines[['top','right','left']].set_visible(False)
...
uploaded_file = st.file_uploader(
    "Choose your lab report CSV file", type=["csv"],
...
if uploaded_file:
    ...
    st.markdown('<div class="info-box">💡 No file yet? Use the sample format shown in the sidebar.</div>', unsafe_allow_html=True)
...
with col1:
    temperature = st.slider("🌡️ Temperature (°C)", min_value=15.0, max_value=45.0, value=28.0, step=0.5)
    st.caption(f"Selected: **{temperature}°C**")
with col2:
    rainfall = st.slider("🌧️ Rainfall (mm)", min_value=200, max_value=900, value=500, step=10)
    st.caption(f"Selected: **{rainfall} mm**")

st.markdown(f"""
<div class="metric-row">
    <div class="metric-card"><div class="metric-value">{temperature}°C</div><div class="metric-label">Temperature</div></div>
    <div class="metric-card"><div class="metric-value">{rainfall}mm</div><div class="metric-label">Rainfall</div></div>
    <div class="metric-card"><div class="metric-value">{'🔴 High' if temperature > 32 else '🟡 Moderate' if temperature > 28 else '🟢 Optimal'}</div><div class="metric-label">Heat Stress</div></div>
    <div class="metric-card"><div class="metric-value">{'🟢 Good' if rainfall > 500 else '🟡 Low' if rainfall > 350 else '🔴 Dry'}</div><div class="metric-label">Water Availability</div></div>
</div>
""", unsafe_allow_html=True)
st.markdown('</div>', unsafe_allow_html=True)
...
# =====================================================================
# RUN PREDICTION — save to session_state
# =====================================================================
if predict_clicked:
    if uploaded_file is None:
        ...
        with st.spinner("🔄 Processing lab report and predicting yields..."):
            try:
                ...
                st.session_state.X_result  = X.copy()
                st.session_state.predicted = True

                # Pre-compute SHAP values and save
                if SHAP_AVAILABLE:
                    explainer = load_explainer(model)
                    if explainer:
                        st.session_state.sv_result = explainer(X)
...
# =====================================================================
# SHOW RESULTS — reads from session_state (persists across reruns)
# =====================================================================
if st.session_state.predicted and st.session_state.df_result is not None:
    df = st.session_state.df_result
    X  = st.session_state.X_result
    sv = st.session_state.sv_result
    ...
    st.markdown(f"""
    <div class="metric-row">
        <div class="metric-card"><div class="metric-value">{avg_yield:.2f}</div><div class="metric-label">Avg Yield (t/ha)</div></div>
        <div class="metric-card"><div class="metric-value" style="color:#1b5e20">{max_yield:.2f}</div><div class="metric-label">Best Yield (t/ha)</div></div>
        <div class="metric-card"><div class="metric-value" style="color:#c62828">{min_yield:.2f}</div><div class="metric-label">Lowest Yield (t/ha)</div></div>
        <div class="metric-card"><div class="metric-value" style="font-size:1rem">{best_line}</div><div class="metric-label">Best Variety</div></div>
    </div>
    """, unsafe_allow_html=True)
    ...
    st.dataframe(
        display_df.style.background_gradient(subset=['Yield (t/ha)'], cmap='Greens')
    ...
    st.markdown('<div class="section-subtitle">SHAP shows how much each feature increased or decreased the yield prediction.</div>', unsafe_allow_html=True)
    ...
    if not SHAP_AVAILABLE:
        ...
        with tab1:
            ...
            fig_summary = make_shap_plot(sv)
            st.pyplot(fig_summary, use_container_width=True)
            plt.close()

            # Markers only recommendation
            mean_shap      = np.abs(sv.values).mean(axis=0)
            ...
            with a mean |SHAP| of <strong>{top_marker_val:.3f} t/ha</strong>.<br><br>
        ...
        with tab2:
            ...
            # ✅ selectbox reads from session_state df — no data loss on rerun
            sample_list = df['Sample_ID'].tolist()
            selected    = st.selectbox("Choose sample", sample_list, key="shap_sample")
